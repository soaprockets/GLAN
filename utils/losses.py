...
class TV_Loss(nn.Module):
    def __init__(self,TV_Loss_weight=1):
        super().__init__()
        self.TVLoss_weight = TV_Loss_weight
    def forward(self,x):
        batch_size = x.size()[0]
        h_x = x.size()[2]
        w_x = x.size()[3]
...
class Total_loss(nn.Module):
    ...
    def __init__(self):
        ...
        self.ssim = SSIMLoss()
        self.rc_loss = CharbonnierLoss()
        # self.blur = Blur(3)
        # self.ms_ssim = MS_SSIM_Loss()
        # self.tv_loss = TV_Loss()

    def forward(self, out, gt):
        # color_loss = torch.abs(out-self.blur(out)-gt-self.blur(gt)).mean()
        ssim_loss = self.ssim(out,gt)
        rc_loss =  self.rc_loss(out,gt)
        # tv_loss = self.tv_loss(out)
        p_loss = self.perception_loss(out, gt)
        loss =  rc_loss+ssim_loss*0.2+p_loss*0.4  
        return loss