...
from transform.dataset_RGB import collate_fn
...
for epoch in range(start_epoch, OPT['EPOCHS'] + 1):
    ...
    for i, data in enumerate(tqdm(train_loader), 0):
        ...
        target, input_,_= data
        target, input_,= target.cuda(), input_.cuda()
        restored = model_restored(input_)
        # Compute loss
        loss = total_loss(restored, target)
    ...
    if epoch % Train['VAL_AFTER_EVERY'] == 0:
        ...
        for ii, data_val in enumerate(val_loader, 0):
            target, input_,file_name= data_val

            file_name_list.append(''.join(file_name))

            target, input_,= target.cuda(), input_.cuda()
            with torch.no_grad():

                restored = model_restored(input_)
                out = restored.permute(0, 2, 3, 1).cuda().data.cpu().numpy()
        ...
        if psnr_val_rgb > best_psnr:
            ...
            for i in range(len(results_list)):
               cv2.imwrite(f'{save_path}/{file_name_list[i]}.png', cv2.cvtColor(np.squeeze(results_list[i])*255,cv2.COLOR_RGB2BGR))
            print("Saving the validation image successfully")
        ...
        # Save the restored validation image
